...
class AfricanaPizzaBuilder : public PizzaBuilder {
public:
	void buildDough() override
	{
		pizza.setDough("No dough");
	}
	void buildSauce() override
	{
		pizza.setSauce("No sauce");
	}
	void buildTopping() override
	{
		pizza.setTopping("No topping");
	}
	Pizza getPizza() const override { return pizza; }
...
int main()
{
	...
	AfricanaPizzaBuilder africanaBuilder;
	cook.makePizza(africanaBuilder);
	Pizza africanaPizza = africanaBuilder.getPizza();
	africanaPizza.displayPizza();
	return 0;
}